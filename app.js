...
const B=[
  ...
  ['rear','High-cable rear-delt fly',12,20,'stack','iso',120],
...
const cues={
  ...
  rear:'Sweep outward without rotating torso.',
...
const howto={
  ...
  rear:'cable rear delt fly proper form tutorial',
...
function defaults(){return{workouts:[],session:null,settings:{compound:180,iso:120,transition:15,sound:true,vibrate:true}}}
function load(){try{return Object.assign(defaults(),JSON.parse(localStorage.getItem('one-set-v1')||'{}'))}catch{return defaults()}}
function save(){localStorage.setItem('one-set-v1',JSON.stringify(d))}
function nextPlan(){let w=d.workouts?.at(-1);return w?.plan==='A'?'B':'A'}
function last(ex){for(let i=d.workouts.length-1;i>=0;i--){let r=d.workouts[i].sets.find(x=>x.id===ex[0]);if(r)return r}return null}
function suggested(ex){let l=last(ex);if(!l)return 0;return l.reps>=ex[3]?l.weight+increment(ex):l.weight}
function increment(ex){return ['incline','lateral','ohp'].includes(ex[0])?5:ex[0]==='dip'?5:10}
function fmt(n){n=Number(n)||0;return Number.isInteger(n)?n:n.toFixed(1)}
function clock(sec){sec=Math.max(0,Math.ceil(sec));return`${String(Math.floor(sec/60)).padStart(2,'0')}:${String(sec%60).padStart(2,'0')}`}
function current(){return R[d.session.plan][d.session.i]}
function restFor(ex){return ex[0]==='ext'?d.settings.transition:ex[5]==='compound'?d.settings.compound:d.settings.iso}
function toastMsg(x){toast.textContent=x;toast.classList.add('on');setTimeout(()=>toast.classList.remove('on'),2200)}
function yt(id){return`https://www.youtube.com/results?search_query=${encodeURIComponent(howto[id]||id+' exercise tutorial')}`}
function howtoLink(id,label='Watch how-to'){return`<a class=howto href="${yt(id)}" target=_blank rel="noopener">${label} ↗</a>`}
function render(){document.body.classList.toggle('session',!!d.session);if(d.session)return renderSession();nav.style.display='grid';if(view==='history')return renderHistory();if(view==='edit')return renderEditWorkout();if(view==='settings')return renderSettings();renderHome()}
function renderHome(){let r=R[plan];app.innerHTML=`<section class=hero><p class=eyebrow>ONE WORKING SET</p><h1>Train hard.<br>Log once.</h1><p class=muted>Loads are prefilled. At failure, tap your rep count - that logs the set, advances the exercise and starts recovery.</p></section><div class=tabs>${['A','B'].map(x=>`<button data-plan=${x} class=${plan===x?'on':''}>Workout ${x}${nextPlan()===x?' · Next':''}</button>`).join('')}</div><section class=card><p class=eyebrow>WORKOUT ${plan}</p><h2>${plan==='A'?'Pre-exhaust + vertical pull':'Dips + horizontal pull'}</h2><div class=meta><span class=pill>${r.length} sets</span><span class=pill>~25 min</span><span class=pill>Offline</span></div><button class=primary data-start>START WORKOUT ${plan}</button></section><ol class=list>${r.map((e,i)=>`<li><span class=num>${i+1}</span><span><strong>${e[1]}</strong><small>${e[4]} · ${howtoLink(e[0],'form video')}</small></span><span class=target>${e[2]}-${e[3]}</span></li>`).join('')}</ol><p class=muted style="font-size:11px">Warm-up sets stay outside the log. Machines/isolations can reach concentric failure; compounds stop at technical failure.</p>`}
function renderSession(){let s=d.session,e=current(),pct=s.i/R[s.plan].length*100;if(s.phase==='rest'&&Date.now()>=s.restEnd){s.phase='ready';save();alertRest()}app.innerHTML=`<div class=top><div><h1>Workout ${s.plan}</h1><span class=muted style="font-size:11px">${s.sets.length} of ${R[s.plan].length} logged</span></div><span class=clock data-elapsed>${clock((Date.now()-s.started)/1000)}</span></div><div class=progress><i style="width:${pct}%"></i></div>${s.phase==='rest'?restHTML(s,e):exerciseHTML(s,e)}${stickyHTML(s,e)}`;clearInterval(tickId);tickId=setInterval(updateClocks,250)}
function exerciseHTML(s,e){let l=last(e);return`<section class="card exercise ${s.phase==='set'?'failure':''}"><p class=eyebrow>EXERCISE ${s.i+1} / ${R[s.plan].length}</p><div class=exercise-title><h2>${e[1]}</h2>${howtoLink(e[0])}</div><span class=pill>${e[2]}-${e[3]} reps</span><div class=last><span>Last time</span><strong>${l?`${fmt(l.weight)} lb × ${l.reps}`:'First entry'}</strong></div>${s.phase!=='set'?`<div class=weight><button data-weight=-1>−</button><label><input data-weightinput type=number inputmode=decimal value="${fmt(s.weight)}"><b>lb</b></label><button data-weight=1>+</button></div>`:`<div style="text-align:center;padding:20px 0"><p class=eyebrow style="color:var(--red)">SET IN PROGRESS</p><strong class=clock data-setclock style="font-size:46px">${clock((Date.now()-s.setStart)/1000)}</strong></div>`}<p class=cue>${cues[e[0]]}</p></section>`}
function stickyHTML(s,e){if(s.phase==='set'){let reps=[];for(let n=Math.max(1,e[2]-2);n<=Math.min(30,e[3]+2);n++)reps.push(`<button data-reps=${n}>${n}</button>`);return`<div class="sticky failure"><p class=failure-log-label>AT FAILURE · TAP ACTUAL REPS</p><div class=repgrid>${reps.join('')}</div><div class=minor><button data-cancel>Cancel set</button><button data-other>Other reps</button></div></div>`}return`<div class=sticky><button class=primary data-startset>${s.phase==='rest'?'START NEXT SET':'START SET'}</button><div class=minor>${s.sets.length?'<button data-undo>Undo last</button>':''}<button data-skip>Skip</button><button data-finish>Finish early</button></div></div>`}
function restHTML(s,e){let rem=(s.restEnd-Date.now())/1000,p=Math.max(0,rem/s.restSec),l=last(e);return`<section class="card rest"><div class=ring data-ring style="--p:${p}"><div><small>${s.restSec<=30?'CHANGEOVER':'RECOVER'}</small><strong data-restclock>${clock(rem)}</strong></div></div><h2 style="font-size:24px">Next: ${e[1]}</h2>${howtoLink(e[0],'Review form')}<p class=muted>${e[2]}-${e[3]} reps · Last: ${l?`${fmt(l.weight)} lb × ${l.reps}`:'first entry'}</p><div class=weight><button data-weight=-1>−</button><label><input data-weightinput type=number inputmode=decimal value="${fmt(s.weight)}"><b>lb</b></label><button data-weight=1>+</button></div></section>`}
...
function alertRest(){beep();if(navigator.vibrate&&d.settings.vibrate)navigator.vibrate([100,60,180]);toastMsg('Recovery complete.')}
